...
export default function TransactionCategories({
  ...
  const { data: income_categories, isLoading: isLoading1 } = useCategories(
    Type.INCOME
  );
  const { data: expense_categories, isLoading: isLoading2 } = useCategories(
    Type.EXPENSE
  );
  const { data: transfer_categories, isLoading: isLoading3 } = useCategories(
    Type.TRANSFER
  );
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      ...
      <div className="flex gap-1 justify-evenly">
        <div
          onClick={() => setActiveType(Type.INCOME)}
        ...
        <div
          onClick={() => setActiveType(Type.EXPENSE)}
          className={`px-4 py-2 flex justify-center items-center gap-1 flex-1 bg-background text-onBackground text-size-4xsm rounded-xl cursor-pointer ${
        ...
        <div
          onClick={() => setActiveType(Type.TRANSFER)}
          className={`px-4 py-2 flex justify-center items-center gap-1 flex-1 bg-background text-onBackground text-size-4xsm rounded-xl cursor-pointer ${
